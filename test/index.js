// enzyme http://airbnb.io/enzyme/docs/api/ReactWrapper/exists.html

require('./common/FormGroupHelper.spec');
require('./common/validations.spec');

require('./components/FormComponent.spec');
require('./components/FormGroup.spec');
require('./components/Checkbox.spec');
require('./components/CheckboxGroup.spec');
require('./components/Input.spec');
require('./components/Email.spec');
require('./components/File.spec');
require('./components/Number.spec');
require('./components/Password.spec');
require('./components/RadioGroup.spec');
require('./components/Range.spec');
require('./components/Reset.spec');
require('./components/Select.spec');
require('./components/Submit.spec');
require('./components/Text.spec');
