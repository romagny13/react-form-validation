...
function generate(paths) {
    var errors = [];
    let files = getFiles(paths.components);
    var componentData = getFiles(paths.components).map(function (componentFileName) {
        try {
            return getComponentData(paths, componentFileName);
        } catch (error) {
            errors.push('An error occurred while attempting to generate metadata for ' + componentFileName + '. ' + error);
        }
    });
    writeFile(paths.output, "module.exports = " + JSON.stringify(errors.length ? errors : componentData));
}

function getComponentData(paths, componentFileName) {
    var componentName = componentFileName.split('.')[0];
    console.log(componentFileName, componentName);
    var content = readFile(path.join(paths.components, componentFileName));
